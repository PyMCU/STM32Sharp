    ...
    private void UpdateFetchCache(uint pc)
    {
        ...
        switch (_currentRegionId)
        {
            ...
            case BusInterconnect.REGION_SRAM:
                ...
                _fetchMask = Bus.MaskSram & ~1u;
                break;
            default:
                // Execution from peripheral/system regions is not modeled via the fetch
                // fast-path; the Run() loop raises a HardFault when _fetchPtr is null.
                _fetchPtr = null;
                break;
        }
    }
